...
def sigmoid(xdata, x0, k):
    ...
    ----------
    x0 : float
        Transition point of the sigmoid function
    k : float
...
class gaussian_param_evolution_redshift(object):
    '''
    Parameters
    ----------
    mu0 : float
        Value of the mean of the gaussian at redshift z=0
    sigma0 : float
        Value of the standard deviation of the gaussian at redshift z=0
    alpha_1 : float
        Value of the first order coefficient of the Taylor expansion for \mu_g(z)
    beta_1 : float
        Value of the first order coefficient of the Taylor expansion for \sigma_g(z)
    '''
    def __init__(self,mu0, sigma0, alpha_1, beta_1):
        self.mu0 = mu0
        self.sigma0 = sigma0
    ...
    def mu_z(self, zdata):
        to_ret = self.mu0 + self.alpha_1*zdata
        return to_ret
    
    def sigma_z(self, zdata):
        to_ret = self.sigma0 + self.beta_1*zdata
        return to_ret

class Truncatedgaussian_redshift(object):
    '''
    Parameters
    ----------
    mu0 : float
    ...
    '''
    def __init__(self,mu0,sigma0,alpha_1,beta_1,mmin,mmax):
        self.mu0 = mu0
        self.sigma0 = sigma0
        self.alpha_1 = alpha_1
        self.beta_1 = beta_1
        self.mmin = mmin
        self.mmax = mmax
        self.G_param = gaussian_param_evolution_redshift(mu0,sigma0,alpha_1,beta_1)
    ...
    def log_pdf(self, x1, x2):
        ...
        mu = self.G_param.mu_z(x2)
        sigma = self.G_param.sigma_z(x2)
        norm_fact= get_gaussian_norm(self.mmin,self.mmax,mu,sigma)
        to_ret = -xp.log(sigma)-0.5*xp.log(2*xp.pi)-0.5*xp.power((x1-mu)/sigma,2.)-xp.log(norm_fact)
        return to_ret
    ...
    def log_cdf(self,x1, x2):
        '''
        x1 : mass
        x2 : redshift
        '''
        xp = get_module_array(x1)
        sx = get_module_array_scipy(x1)
        mu = self.G_param.mu_z(x2)
        sigma = self.G_param.sigma_z(x2)
        max_point = (x1-mu)/(sigma*xp.sqrt(2.))
        min_point = (self.mmin-mu)/(sigma*xp.sqrt(2.))
        norm_fact= get_gaussian_norm(self.mmin,self.mmax,mu,sigma)
        toret = np.log((0.5*sx.special.erf(max_point)-0.5*sx.special.erf(min_point))/norm_fact)
        return toret
    ...
    def cdf(self,x1,x2):
        ...
        '''
        xp = get_module_array(x1)
        return xp.exp(self.log_cdf(x1,x2))

class PLsigmoid(object):
    '''
    Parameters
    ----------
    alpha : float
    ...
    '''
    def __init__(self,alpha,mmin,mmax,delta_m,mu0,sigma0,alpha_1,beta_1,x0,k):
        self.alpha = alpha
        self.mmin = mmin
        self.mmax = mmax
        ...
        #self.Sigmoid = Sigmoid(x0,k)
        self.TG_zevol = Truncatedgaussian_redshift(mu0,sigma0,alpha_1,beta_1,mmin,mmax)
        self.PL = PowerLaw(mmin,mmax,alpha)
    ...
    def pdf(self,x1,x2):
        ...
        x2 : redshift
        '''
        to_ret = sigmoid(x2,self.x0,self.k)*self.PL.pdf(x1) + (1-sigmoid(x2,self.x0,self.k))*self.TG_zevol.pdf(x1,x2)
        return to_ret 
