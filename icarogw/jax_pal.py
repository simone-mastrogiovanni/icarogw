...
class RegularGridInterpolator:
    ...
    **Evaluate a function on the points of a 3-D grid**
    ...
    >>> x = jnp.linspace(1, 4, 11)
    >>> y = jnp.linspace(4, 7, 22)
    >>> z = jnp.linspace(7, 9, 33)
    >>> xg, yg ,zg = jnp.meshgrid(x, y, z, indexing='ij', sparse=True)
    >>> data = f(xg, yg, zg)
    ...
    **Interpolate and extrapolate a 2D dataset**
    ...
    >>> x, y = jnp.array([-2, 0, 4]), jnp.array([-2, 0, 2, 5])
    >>> def ff(x, y):
    ...     return x**2 + y**2
    ...
    >>> xx = jnp.linspace(-4, 9, 31)
    >>> yy = jnp.linspace(-4, 9, 31)
    ...
    def __init__(self, points, values, method="linear", bounds_error=True,
        ...
        if self._descending_dimensions:
            self.values = jnp.flip(values, axis=self._descending_dimensions)
    ...
    def __call__(self, xi, method=None):
        ...
        >>> import numpy as jnp
        >>> x, y = jnp.array([0, 1, 2]), jnp.array([1, 3, 7])
        >>> def f(x, y):
        ...     return x**2 + y**2
        >>> data = f(*jnp.meshgrid(x, y, indexing='ij', sparse=True))
        >>> from scipy.interpolate import RegularGridInterpolator
    ...
    def _prepare_xi(self, xi):
        ...
        # find nans in input
        nans = jnp.any(jnp.isnan(xi), axis=-1)
    ...
    def _evaluate_linear(self, indices, norm_distances):
        ...
        for h in hypercube:
            ...
            for w in weights:
                weight = weight * w
            term = jnp.asarray(self.values[edge_indices]) * weight[vslice]
    ...
    def _evaluate_spline(self, xi, method):
        ...
        result = jnp.empty(shape, dtype=self.values.dtype)
        for j in range(m):
...
def interpn(points, values, xi, method="linear", bounds_error=True,
    ...
    >>> z = jnp.linspace(0, 6, 7)
    >>> points = (x, y, z)
    >>> values = value_func_3d(*jnp.meshgrid(*points, indexing='ij'))

    Evaluate the interpolating function at a point

    >>> point = jnp.array([2.21, 3.12, 1.15])
    ...
    if method in ["linear", "nearest", "slinear", "cubic", "quintic", "pchip"]:
        ...
        idx_valid = jnp.all((grid[0][0] <= xi[:, 0], xi[:, 0] <= grid[0][-1],
                            grid[1][0] <= xi[:, 1], xi[:, 1] <= grid[1][-1]),
                           axis=0)
        result = jnp.empty_like(xi[:, 0])

        # make a copy of values for RectBivariateSpline
        interp = RectBivariateSpline(points[0], points[1], values[:])
        result[idx_valid] = interp.ev(xi[idx_valid, 0], xi[idx_valid, 1])
        result[jnp.logical_not(idx_valid)] = fill_value
